...
function Loading({ isLoading }: { isLoading: boolean }) {
    ...
    return <Icon icon="solar:refresh-bold" size="md" className={`absolute rotate-360 animate-spin ${visibility}`} />;
}
...
export default function Button({
    ...
    let style = "relative px-4 py-0 font-semibold stroke-1 stroke-inherit inline-flex items-center justify-center rounded-xl whitespace-nowrap"
    ...
    if (!!leftIcon || !!rightIcon){
        leftIconNode = leftIcon ? <Icon icon={leftIcon} size={iconSize} className="mr-2" /> : null;
        rightIconNode = rightIcon ? <Icon icon={rightIcon} size={iconSize} className="ml-2" /> : null;
    }
